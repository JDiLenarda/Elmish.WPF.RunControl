...
module SameLoop =
  (*  When performed in the command part of the loop, such a call blocks the update loop and let the messages queuing,
      unless you run it in an async mode and in the application thread dispatcher.
  ...
  let showDialog parent model dispatch = 
      let selector = CultureSelector()
      selector.Owner <- parent
      parent |> Ctx.ofWindow |> Ctx.bindTo selector
...
module SelfLoop =
  ...
  let private _showDialog parent program =      
      let window = CultureSelector()      // take note that CultureSelector has a few lines of C# code-behind
      window.Owner <- parent
      Program.bindFrameWorkElement window program
      if window.ShowDialog().Value then Choice1Of2 (window |> Ctx.ofWindow |> Ctx.currentModel<SubModel>)
      else Choice2Of2 ()

  let showDialog parent culture dispatch =
      let init () = culture
      ...
      |> _showDialog parent |> function
      | Choice1Of2 model ->  ChangeCulture model |> dispatch
      | Choice2Of2 _ -> ()
...
let bindings parent _ dispatch =
  let callSameLoopDialog' = SameLoop.showDialog parent
  let callSelfLoopDialog' = SelfLoop.showDialog parent
